...
def profile(req):
    ...
    user_restaurants = (
        Restaurant.objects.filter(user=custom_user).all()
        if Restaurant.objects.filter(user=custom_user).all().exists()
        else None
    )
    custom_user = {
...
def generate_email_otp(req):
    ...
    if not user.is_email_verified:
        ...
        email_otp = OTP.objects.create(user=user, type_Of_otp="email", otp=otp)
        email_otp.save()
        try:
            ...
            print(e)
            JsonResponse(
                {"status": "error", "message": "Server Not Responsing" + str(e)},
                status=500,
            )
        return JsonResponse(
    ...
    return JsonResponse(
        {"status": "error", "message": "Email already verified"}, status=400
    )
...
def change_profile_picture(req):
    ...
    if not req.user.is_authenticated:
        return JsonResponse(
            {"status": "error", "message": "Unauthorized User"}, status=401
        )

    if req.FILES.get("profile_picture") is None:
        return JsonResponse(
            {"status": "error", "message": "Profile Picture is required"}, status=400
        )
    ...
    try:
        ...
        if profile_picture_url is None:
            return JsonResponse(
                {"status": "error", "message": "Server Not Responsing"}, status=500
            )
...
def update_user_details(req):
    ...
    customUser = CustomUser.objects.only(
        "user_type", "date_of_birth", "biography", "gender", "first_name", "last_name"
    ).get(username=req.user)
    context = {
